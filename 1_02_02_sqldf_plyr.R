###################################################################
rm(list = ls())

##############################
install.packages("sqldf")
library(sqldf)

##############################
iris
sqldf("select * from iris")
sqldf("select * from iris limit 10")
# like 사용 시에는 ''사용
sqldf("select count(*) from iris where Species like 'se%'") 
sqldf("select * from iris where `Petal.Length` >= 6.5") 
sqldf("select * from iris where [Petal.Length] >= 6.5") 
subset(iris, Petal.Length >= 6.5)

###############################
# 데이터 분리하고 처리한 다음 결합하는 필수적 기능 제공 
# apply에 기반해 출력변수를 배열로 치환해주는 패키지
# 앞의 글자는 입력 데이터 형태 
# 둘째 글자는 출력 데이터 형태
# d(dataframe), a(array), l(list) 3 x 3개의 경우의 수가 존재
set.seed(1) # 난수 고정

# 0에서 1 사이의 균등 분포 난수(random uniform distribution) 3 개 생성
?runif
runif(3, min = 0, max = 1) 
runif(20, 0, 20) # 0에서 20 사이의 난수 20 개 생성

rep(2012:2014, each=6) # 총 18개
round(runif(9, 0, 20)) # 총 9개
round(runif(9, 0, 20), 2) # 총 9개

# year란 변수에는 6개씩 2012부터 2014까지 넣고(총 18개), 
# count란 변수에는 0에서 20사이 정수 9개
# 뒤의 빈부분은 다시 반복해서 입력
set.seed(1) # 난수 고정
d <- data.frame(year = rep(2012:2014, each=6), 
                count = round(runif(9, 0, 20)))
print(d)

#######################################
# 패키지 설치
install.packages("plyr")
library(plyr)

# 함수 apply()
# apply 계열 함수 중 가장 기본이 되는 함수
# 행(Row) 또는 열(Column) 단위의 연산을
# 쉽게 할 수 있도록 지원하는 함수 
# MARGIN이라는 함수 인자에 어떤 단위로 연산을
# 할 것인지 1 또는 2로 표시하는데, 
# 1이 행 단위 연산이고 2가 열 단위 연산
# apply의 Input Data는 배열(Array), 
# 매트릭스(Matrix) 등 모두 같은 변수형을
# 가진 값으로 이루어진 데이터 타입만 가능
# 모두 같은 변수형을 가진 데이터프레임(Data Frame)도 가능
# apply 함수의 실행 결과는 # 매트릭스나 벡터(Vector)로 출력

?apply()
x <- cbind(x1 = 3, x2 = c(4:1, 2:5))
x
## mean, 1: 행
apply(x, 1, mean)
## sum 적용, 2: 열
apply(x, 2, sum)

# 변동계수 계산하는 함수를 year 변수의 각 값을 기준으로 구함
# 變動係數 / coefficient of variation
# 변동계수 또는 상대 표준편차(relative standard deviation, RSD)
# 표준편차를 표본평균이나 모평균 등 산술 평균으로 나눈 것
# 측정 단위가 다른 자료를 비교할 때 사용
# 즉, "변동계수 = 표준편차 / 평균"

d
# 도움말 다음 또는 단어선택 + F1
#?ddply
ddply(d, "year", function(x) {
  mean.count = mean(x$count)
  sd.count = sd(x$count)
  cv = sd.count / mean.count
  data.frame(cv.count = cv)
})

# 좀 더 복잡한 예제로 transform 옵션과 summaries 옵션 동시 사용
# 새로 생긴 변수, mean.count만 표시
# summarize는 요약된 결과만 출력 → 그룹별 1행씩
ddply(d, "year", summarise, mean.count = mean(count))

# 새로 생긴 변수, total.count만 표시
ddply(d, "year", summarise, total.count = sum(count)) 

# transform 옵션이 있으면 계산에 사용된 변수도 출력
# transform은 원래 행 수 그대로 유지 + 파생 변수 추가
# 즉, transform은 그룹별 요약값을 원래 데이터 각 행에 추가하는 방식
ddply(d, "year", transform, total.count = sum(count)) 

# end

###########################################################
# 자기주도학습
# 도움말 문서 예제 코드
dfx <- data.frame(
  group = c(rep('A', 8), rep('B', 15), rep('C', 6)),
  sex = sample(c("M", "F"), size = 29, replace = TRUE),
  age = runif(n = 29, min = 18, max = 54)
)
dfx

# Note the use of the '.' function to allow
# group and sex to be used without quoting
# 구룹당 표시
ddply(dfx, .(group, sex), summarize,
      mean = round(mean(age), 2),
      sd = round(sd(age), 2))

# 모든 행에 사용된 열도 표시
ddply(dfx, .(group, sex), transform,
      mean = round(mean(age), 2),
      sd = round(sd(age), 2))

# 모든 행에 표시
ddply(dfx, .(group, sex), 
      mean = round(mean(age), 2),
      sd = round(sd(age), 2))

###################################################################
